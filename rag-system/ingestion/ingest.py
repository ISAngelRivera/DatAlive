...
class DocumentIngestionPipeline:
    ...
    async def _ingest_single_document(self, file_path: str) -> IngestionResult:
        ...
        # Chunk the document (simplified approach to avoid async issues)
        chunks = self._simple_chunk_document(
    ...
    def _simple_chunk_document(
        ...
        """
        Simple document chunking without async complexity.
        ...
        while start < len(content):
            ...
            if end >= len(content):
                ...
                chunk_end = end
                for i in range(end, max(start + 100, end - 200), -1):
                    if content[i] in '.!?\n':
                        chunk_end = i + 1
                        break
            ...
            if chunk_content.strip():
                chunk = DocumentChunk(
                    ...
                    token_count=len(chunk_content) // 4  # Rough estimation
                )
                chunks.append(chunk)
                chunk_index += 1
